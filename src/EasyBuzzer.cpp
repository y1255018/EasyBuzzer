...
/* Beep at a given frequency an specific number of beeps, with callback
 * functionality. */
void EasyBuzzerClass::beep(unsigned int frequency, unsigned int beeps,
                           void (*finishedCallbackFunction)()) {
  beep(frequency, mOnDuration, mOffDuration, 1, mPauseDuration, beeps,
       finishedCallbackFunction);
}
/* Beep sequence at a given frequency. */
void EasyBuzzerClass::beep(unsigned int frequency,
                           unsigned int const onDuration,
                           unsigned int const offDuration, byte const beeps,
...
void EasyBuzzerClass::singleBeep(unsigned int frequency,
                                 unsigned int duration) {
  beep(frequency, duration, mOffDuration, 1, mPauseDuration, 1, NULL);
}
/* Start beeping at a given frequency, for an specific duration, with callback
 * functionality. */
void EasyBuzzerClass::singleBeep(unsigned int frequency, unsigned int duration,
                                 void (*finishedCallbackFunction)()) {
  beep(frequency, duration, mOffDuration, 1, mPauseDuration, 1,
       finishedCallbackFunction);
}
/* Stop beeping. */
...
void EasyBuzzerClass::setPin(unsigned int pin) { mPin = pin; }
/* Set On duration. */
...
/* Set Buzzer Volume. */
void EasyBuzzerClass::setVolume(unsigned int volume) { mVolume = volume; }
...
void EasyBuzzerClass::update() {
  ...
  unsigned int sequenceDuration =
      blinkDuration * mBeeps + mPauseDuration - mOffDuration;
...
#if defined ESP32
  if (timeInSequence < blinkingDuration &&
      timeInSequence % blinkDuration < mOnDuration) {
    ledcAttachPin(mPin, mChannel);

    // ledcWriteTone(mChannel, mFreq);
    ledcWriteTone2(mChannel, mFreq, mVolume);
